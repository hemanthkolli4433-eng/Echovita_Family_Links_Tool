...
# ---------- Optional input file picker (no output picker) ----------
def pick_open_files():
    """Pick ONE or MULTIPLE Excel files using a file dialog."""
    try:
        ...
        paths = filedialog.askopenfilenames(
            title="Select input Excel file(s)",
        ...
        return []  # e.g., headless environment


# ---------- End popup ----------
def show_end_popup(title: str, message: str, is_error: bool = False):
    """Show a final popup message (best-effort; won't crash if tkinter not available)."""
    try:
        import tkinter as tk
...
def resolve_single_output_path(first_input_path: str) -> str:
    """ALWAYS write one combined file next to the FIRST input file."""
    input_dir = os.path.dirname(os.path.abspath(first_input_path)) or "."
    return os.path.join(input_dir, "Combined_Excel.xlsx")
...
def main():
    ...
    # Input from CLI or file picker (MULTI supported)
    input_paths = argv if len(argv) >= 1 else pick_open_files()

    if not input_paths:
        print("No input selected. Tip: run `python script.py <input1.xlsx> <input2.xlsx> ...`.")
        sys.exit(1)

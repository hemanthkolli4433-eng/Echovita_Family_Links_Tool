...
# --- Tkinter file dialog (no GUI window shown other than the dialogs) ---
try:
...
def main():
    # GitHub / command-line mode: use file path argument first
    if len(sys.argv) >= 2:
        in_path = sys.argv[1]
        root = None

    # Headless fallback if Tk unavailable
    elif tk is None or filedialog is None:
        print("GUI file dialog not available. Run with Tkinter or pass a file path as an argument.")
        print("Usage: python Echovita_Dates_Cleanup_Tool.py /path/to/workbook.xlsx")
        sys.exit(2)

    # Local Windows mode: show file picker
    else:
        root = tk.Tk()
        root.withdraw()
        root.lift()
        ...
        if not in_path:
            if messagebox:
                messagebox.showinfo("Excel Cleanup Tool", "No file selected. Exiting.", parent=root)
            else:
    ...
    try:
        ...
        if messagebox and 'root' in locals() and root is not None:
    ...
    msg = f"Cleaned file saved:\n{out_path}"

    if messagebox and 'root' in locals() and root is not None:
        messagebox.showinfo("Excel Cleanup Tool", msg, parent=root)

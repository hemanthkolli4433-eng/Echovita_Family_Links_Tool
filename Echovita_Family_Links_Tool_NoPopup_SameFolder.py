...
def get_base_dir():
    """
    Returns the folder where the .py file or .exe is located.
    Input Excel will be read from this folder.
    Output Excel will also be saved in this folder.
    """
...
def find_input_file():
    """
    Automatically picks the input Excel from the same folder where code/exe runs.

    Rules:
    - Looks only in the code/exe folder.
    - Ignores temporary Excel files starting with ~$.
    - Ignores the output file name.
    - If multiple input Excel files are found, it picks the newest modified file.
    """
    ...
    for name in os.listdir(base_dir):
        ...
        if not lower_name.endswith(".xlsx"):
            continue
        if lower_name == OUTPUT_FILENAME.lower():
            continue
    ...
    if not files:
        raise Exception(
            ...
            f"Place your input .xlsx file in this folder:\n{base_dir}\n\n"
            f"Do not name the input file as {OUTPUT_FILENAME}."
        )

    # Pick newest input file if more than one Excel is present.
...
def get_output_file_path(input_file_path=None):
    # Save output in the same folder where code/exe runs.
...
def handle_captcha_if_present(driver):
    while True:
        ...
        if CAPTCHA_TEXT in body_text:
            wait_for_user("Captcha detected. Solve captcha in Chrome, then come back here.")
            time.sleep(2)
            continue
        break
...
def handle_server_or_page_error(driver, ws, wb, output_path):
    ...
    # Case 2: HTTPS / server error - console wait for user reload
    server_error_words = [
        "HTTP ERROR",
        "This page isn’t working",
        "This site can’t be reached",
    ...
    if any(word.lower() in body_text.lower() for word in server_error_words):
        wait_for_user(
            "HTTPS/server error found. Please reload the page manually in Chrome, then come back here."
        )
        time.sleep(2)
        return "RECHECK"
...
def run():
    ...
    try:
        ...
        print(f"Input Excel found: {input_file_path}")
        ...
        if not input_rows:
            print("No Echovita links found in column 4.")
            return
        ...
        save_workbook_safe(wb, output_path)
        print(f"\nAll URLs Done. Output saved in Excel:\n{output_path}")
